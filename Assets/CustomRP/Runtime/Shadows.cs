...
public class Shadows
{
	...
	{
		if (ShadowedDirectionalLightCount < maxShadowedDirectionalLightCount && light.shadows != LightShadows.None && light.shadowStrength > 0 &&
	...
	void RenderDirectionalShadows(int index, int split, int tileSize)
	{
		...
		for (int i = 0; i < cascadeCount; i++)
		{
			...
			// set up the culling spheres for all lights
			// only need to do it once since they are identical for all lights
			if (index == 0)
			{
			...
			//set the conversion matrix from world space to the atlas space of each spit of the shadow atlas of each light
			dirShadowMatrices[tileIndex] = ConvertToAtlasMatrix(projectionMatrix * viewMatrix, SetTileViewport(tileIndex, split, tileSize), split);
			buffer.SetViewProjectionMatrices(viewMatrix, projectionMatrix);
			// add depth bias to mitigate shadow acne
